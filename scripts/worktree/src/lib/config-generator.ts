...
VITE_DEV_PORT=${ports.frontend}
VITE_MCP_PLAYGROUND_PORT=${ports.mcpPlayground ?? ports.frontend + 100}
VITE_AUTH_BYPASS=true
`;
...
/**
 * Generate the .env file for the MCP playground app.
 */
export function generateMcpPlaygroundEnvFile(worktreePath: string, ports: Ports, worktreeId: string): void {
  const envPath = path.join(worktreePath, 'apps', 'mcp-playground', '.env');
  const envDir = path.dirname(envPath);
  ensureDir(envDir);

  const playgroundPort = ports.mcpPlayground ?? ports.frontend + 100;
  const content = `# Auto-generated by worktree setup
# Worktree ID: ${worktreeId}
# Generated: ${timestamp()}
# DO NOT COMMIT
...
`;
  ...
  fs.writeFileSync(envPath, content);
  console.log(`Generated MCP playground .env file at ${envPath}`);
}
...
export function generateDevcontainerJson(
  ...
  const config = {
    ...
    containerEnv: {
      ...
      VITE_MCP_PLAYGROUND_PORT: String(ports.mcpPlayground ?? ports.frontend + 100),
    ...
    features: {},
    forwardPorts: [ports.frontend, ports.mcpPlayground ?? ports.frontend + 100, ports.backend, ports.database],
  };
...
services:
  app:
    ...
    ports:
      ...
      - "${ports.mcpPlayground ?? ports.frontend + 100}:${ports.mcpPlayground ?? ports.frontend + 100}"
...
export function generateAllConfigs(
  ...
  generateMcpPlaygroundEnvFile(worktreePath, ports, worktreeId);
