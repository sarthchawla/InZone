---
name: "source-command-fix-all-tests"
description: "Run all repo tests with a team of agents, then fix failures"
---

# source-command-fix-all-tests

Use this skill when the user asks to run the migrated source command `fix-all-tests`.
...
Use this skill when the user asks to run and fix all tests. Treat any text after the command name as the target package, branch, diff range, or test scope. Use normal Codex tool safety and project instructions for command execution; the old command tool allowlist is guidance, not a Codex permission boundary.
