---
name: "source-command-create-missing-tests"
description: "Find and create missing tests for current branch changes vs master"
---

# source-command-create-missing-tests

Use this skill when the user asks to run the migrated source command `create-missing-tests`.
...
Use this skill when the user asks to find or create missing tests. Treat any text after the command name as the target branch, diff range, feature, or file scope. Resolve placeholders from the user's request and repository context before running commands, and read referenced files explicitly; the old command tool allowlist is guidance, not a Codex permission boundary.
