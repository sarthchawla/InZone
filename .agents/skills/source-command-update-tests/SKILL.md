---
name: "source-command-update-tests"
...
# source-command-update-tests

Use this skill when the user asks to run the migrated source command `update-tests`.
...
Use this skill when the user asks to update tests. Treat any text after the command name as the target branch, diff range, feature, or file scope. Resolve placeholders from the user's request and repository context before running commands; the old command tool allowlist is guidance, not a Codex permission boundary.
