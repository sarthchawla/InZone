...
Use this skill when the user asks to fix CI. Treat any text after the command name as the CI target, branch, PR/MR, or failure context. Resolve placeholders from the user's request and repository context before running commands; do not depend on slash-command template expansion.
